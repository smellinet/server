...
bool TradeStatusAction::Execute(Event event)
{
    ...
    if (trader != master)
    {
		...
		MasterPlayer *tradMaster = ObjectAccessor::FindMasterPlayer(trader->GetObjectGuid());

		botMaster->Whisper("I'm kind of busy now", LANG_UNIVERSAL, tradMaster);
    }
    ...
    {
        if (!bot->isInFront(trader, sPlayerbotAIConfig.sightDistance, M_PI_F / 2.0))
