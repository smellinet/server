...
bool PlayerbotAI::TellMasterNoFacing(string text, PlayerbotSecurityLevel securityLevel)
{
    ...
    if (sPlayerbotAIConfig.whisperDistance && !bot->GetGroup() && sRandomPlayerbotMgr.IsRandomBot(bot) &&
        ...
        return false;
	MasterPlayer *botMaster = ObjectAccessor::FindMasterPlayer(bot->GetObjectGuid());
	MasterPlayer *toMaster = ObjectAccessor::FindMasterPlayer(bot->GetObjectGuid());

	botMaster->Whisper(text, LANG_UNIVERSAL, toMaster);
    return true;
}
...
bool PlayerbotAI::TellMaster(string text, PlayerbotSecurityLevel securityLevel)
{
    ...
    {
        if (!bot->isInFrontInMap(master, sPlayerbotAIConfig.sightDistance, M_PI / 2))
...
bool PlayerbotAI::CastSpell(uint32 spellId, Unit* target)
{
    ...
    if (!bot->isInFrontInMap(faceTo, sPlayerbotAIConfig.sightDistance))
    {
